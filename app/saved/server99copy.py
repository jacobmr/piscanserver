...
class SettingsForm(FlaskForm):
     remote = SelectField('Remote', choices=[('dropbox', 'Dropbox'), ('google', 'Google Drive')], validators=[DataRequired()])
    ...
    brightness = SelectField('Brightness', choices=[(str(i), str(i)) for i in range(-100, 100)], validators=[DataRequired()])
    contrast = SelectField('Contrast', choices=[(str(i), str(i)) for i in range(-100, 100)], validators=[DataRequired()])
...
def write_settings_to_file(settings_dict):
    with open('scan-settings_HP.txt', 'w') as f:
        for key, value in settings_dict.items():
            f.write('%s=%s\n' % (key, value))
        f.write('REMOTE=%s\n' % settings_dict.get('remote', 'dropbox'))
    ...
    if api_key:
        ...
        if settings_dict:
            try:
                ...
                return jsonify({'error': 'Failed to update scanner settings: %s' % str(e)}), 500
        else:
